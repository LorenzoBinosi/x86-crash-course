...
\begin{frame}[fragile]{Using {\tt EBP} to access local variables}
  \begin{columns}
    \begin{column}{0.45\linewidth}
      \begin{itemize}
        \item {\tt EBP} is the address of the base of the current function's frame
        \item It is used to easily access local variables
        \item Local variables are stored in the stack frame, at a lower address than {\tt EBP}: \textit{negative offsets}
      \end{itemize}
    \end{column}
    \begin{column}{0.4\linewidth}
      \par
      ...
      \begin{tikzpicture}[scale=.7, font=\footnotesize]
        \stacktop{}
        \cell{Local variable 3}\cellptr{{\tt EBP - 0xc}} 
        \cell{Local variable 2}\cellptr{{\tt EBP - 0x8}} 
        \cell{Local variable 1}\cellptr{{\tt EBP - 0x4}} 
        \cell{Saved {\tt EBP}}\cellptr{{\tt EBP}}
        \bcell{Return addr (s{\tt EIP})}
        \bcell{Arguments}
        \bcell{Local variables}
        \stackbottom
...
\begin{frame}[fragile]{Using {\tt EBP} to access function arguments}
  \begin{columns}
    \begin{column}{0.45\linewidth}
      \begin{itemize}
        \item Depending on the calling convention (more on this later), {\tt EBP} may be used to access function arguments
        \item Function arguments are stored in the caller's frame, at a higher address than {\tt EBP}: \textit{positive offsets}
      \end{itemize}
    \end{column}
    \begin{column}{0.4\linewidth}
      \par
      ...
      \begin{tikzpicture}[scale=.7, font=\footnotesize]
        ...
        \cell{Saved {\tt EBP}}\cellptr{{\tt EBP}}
        \bcell{Return addr (s{\tt EIP})}
        \bcell{Argument 1}\cellptr{{\tt EBP + 0x4}}
        \bcell{Argument 2}\cellptr{{\tt EBP + 0x8}}
        \bcell{Argument 3}\cellptr{{\tt EBP + 0xc}}
        \bcell{Local variables}
        \stackbottom
      \end{tikzpicture}
      {\scriptsize High addresses ({\tt 0xbfffffff})}
